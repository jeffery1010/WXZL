...
function initMacTable() {
    var model = $('#tbxMacModelDefault').val();
    var wsIdList = $('#tbxMacWSDefault').val();
    var lineIdList = $('#tbxMacLineDefault').val();
    $('.easyui-datagrid[title="SearchMac"]').datagrid({
        //分页
        fitColumns: true,
        singleSelect: false,
        url: '../../Mac/GetMacForTime',
        queryParams: { model: model, wsIdList: wsIdList, lineIdList: lineIdList },
        pagination: true,
        rownumbers: true,
        pagePosition: 'bottom',
        pageSize: 10,
