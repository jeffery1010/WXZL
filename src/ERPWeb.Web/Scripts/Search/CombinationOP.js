$(function () {
    loadCombinationOPWhere();
    initCombinationOPClick();
});

function initCombinationOPTable() {
    var param = getParamCombinationOP();
    $('#SearchCombinationOPTable').datagrid({
        //分页
        fitColumns: true,
        singleSelect: false,
        idField: "id",
        ...
        onLoadSuccess: function () {
            var selectedList = $('#CombinationOPIdList').val();
        ...
        onCheck: function (index, row) {
            ...
            $('#CombinationOPIdList').val(selectedList);
        },
...
function initCombinationOPClick() {
    $('#btnCombinationOPSearch').click(function () {
        var $table = $('div#indexCombinationOP').siblings('div.divWhere').first().children('table');
        ...
        initCombinationOPTable();
    });
    $('#btnCombinationOPClear').click(function () {
        ...
        $('#CombinationOPIdList').val('');
        $('#SearchCombinationOPTable').datagrid('clearSelections');
        $('#SearchCombinationOPTable').datagrid('loadData', { total: 0, rows: [] });
    });
    $('#btnCombinationOPDownload').click(function () {
        var rows = $('table#SearchCombinationOPTable').datagrid('getSelections');
        debugger;
        if (rows.length < 1) { show('请选择至少一行！'); return; }
        var params = getParamCombinationOP();
    ...
    $('#btnCombinationOPDownloadAll').click(function () {
        //参
        var param = getParamCombinationOP();
        var data = ajaxSame('../../Search/DownloadAllForTime', param, 'post', 'json');
        console.info(data);
        if (data.Code == 200) {
            window.location = '../../Search/ReturnZip' + data.Message;
        } else {
...
function loadCombinationOPWhere() {
    var data = ajaxSame('../../Search/Where', { type: 'OP' }, 'get', 'html');
    $('div.divWhere').append(data);
    
}

function getParamCombinationOP() {
    var $table = $('div#indexCombinationOP').siblings('div.divWhere').first().children('table');
...
function imgMouseMove(obj) {
    var docId = 'imgCombinationOPMaxShow';
