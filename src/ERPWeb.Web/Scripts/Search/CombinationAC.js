$(function () {
    loadCombinationACWhere();
    initCombinationACClick();
});

function initCombinationACTable() {
    var param = getParamCombinationAC();
    $('#SearchCombinationACTable').datagrid({
        //分页
        fitColumns: true,
        singleSelect: false,
        idField: "id",
        ...
        onLoadSuccess: function () {
            var selectedList = $('#CombinationACIdList').val();
        ...
        onCheck: function (index, row) {
            ...
            $('#CombinationACIdList').val(selectedList);
        },
...
function initCombinationACClick() {
    $('#btnCombinationACSearch').click(function () {
        var $table = $('div#indexCombinationAC').siblings('div.divWhere').first().children('table');
        ...
        initCombinationACTable();
    });
    $('#btnCombinationACClear').click(function () {
        ...
        $('#CombinationACIdList').val('');
        $('#SearchCombinationACTable').datagrid('clearSelections');
        $('#SearchCombinationACTable').datagrid('loadData', { total: 0, rows: [] });
    });
    $('#btnCombinationACDownload').click(function () {
        var rows = $('table#SearchCombinationACTable').datagrid('getSelections');
        debugger;
        if (rows.length < 1) { show('请选择至少一行！'); return; }
        var params = getParamCombinationAC();
    ...
    $('#btnCombinationACDownloadAll').click(function () {
        var params = getParamCombinationAC();
...
function loadCombinationACWhere() {
    var data = ajaxSame('../../Search/Where', { type: 'AC' }, 'get', 'html');
    $('div.divWhere').append(data);

}

function getParamCombinationAC() {
    var $table = $('div#indexCombinationAC').siblings('div.divWhere').first().children('table');
...
function imgMouseMove(obj) {
    var docId = 'imgCombinationACMaxShow';
