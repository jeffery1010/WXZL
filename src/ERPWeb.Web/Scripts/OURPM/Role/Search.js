...
function initRoleTable() {
    ...
    var isvalid = -1;
    var isenable = -1;
    $('.easyui-datagrid[title="SearchPartialRole"]').datagrid({
        //分页
        fitColumns: true,
        singleSelect: true,
        url: '/Role/GetRole',
        queryParams: { RoleCode: code, RoleName: name, IsVisible: isvalid, IsEnable: isenable },
        pagination: true,
        rownumbers: true,
        pagePosition: 'bottom',
        pageSize: 10,
...
//重新加载表格
function reloadRoleTable(code, name, isvalid,isenable) {
    $('.easyui-datagrid[title="SearchPartialRole"]').datagrid('load', {
        RoleCode: code,
        RoleName: name,
        IsVisible: isvalid,
        IsEnable: isenable,
...
function clickSearch(obj) {
    ...
    //var code = $(obj).siblings('.easyui-textbox[title="code"]').first().val();
    ...
    reloadRoleTable(code, name, isvalid, isenable);//重新加载表格
}
