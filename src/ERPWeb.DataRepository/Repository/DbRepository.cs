...
namespace ERPWeb.DataRepository
{
    ...
    public class DbRepository : IRepository
    {
        ...
        public List<T> GetListByProc<T>(string procName, List<DbParameter> parameters) where T : class, IModelDecorator<T>, new()
        {
            DataTable dt= ExecuteProcedureForDataTable(procName, parameters);
           return new T().PopModelList(dt);
        }
        public AjaxResult SubmitDataByProc<T>(string procName, List<DbParameter> parameters) where T : class, IModelDecorator<T>, new()
        {
            DataTable dt = ExecuteProcedureForDataTable(procName, parameters);
            return new T().ParseSubmitResult(dt);
        }
        #endregion
      ...
      public  DataSet ExecuteProcedureForDataSet(string proc, List<DbParameter> parameters) {
        ...
        {
            DataSet ds = ExecuteProcedureForDataSet(proc, parameters);
