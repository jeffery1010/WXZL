    ...
    public class WcfHost<IService, Service> : IWcfHost where Service : IService
    {
        ...
        private void InitService()
        {
            ...
            if (OpenSecurity)
            {
                ...
                _serviceHost.Credentials.UserNameAuthentication.CustomUserNamePasswordValidator = new CustUserNamepwdValidator(UserNo, Password);
            }
        }
        ...
        /// 用户名
        /// </summary>
        public string UserNo { get => string.IsNullOrEmpty(_UserNo) ? WcfHelper.UserNo : _UserNo; set => _UserNo = value; }
        ...
        {
            public CustUserNamepwdValidator(string UserNo, string password)
            {
                _UserNo = UserNo;
                _password = password;
            }
            private string _UserNo { get; }
            private string _password { get; }
            public override void Validate(string UserNo, string password)
            {
                if (UserNo != _UserNo || password != _password)
                {
