...
namespace ERPWeb.Util.Wcf
{
    ...
    public static class WcfClient
    {
        ...
        /// <returns></returns>
        public static T GetService<T>(string serviceUrl, bool openSecurity, string UserNo,string password)
        {
            ...
            if (openSecurity)
            {
                ...
                if (string.IsNullOrEmpty(UserNo))
                    UserNo = WcfHelper.UserNo;
                if (string.IsNullOrEmpty(password))
                    password = WcfHelper.Password;

                factory.Credentials.UserName.UserName = UserNo;
